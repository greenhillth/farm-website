...
OPTIMAL = {"OM": (3.25, 5.2), "P1": (40, 90), "K": (245, 400),
           "MG": (220, 440), "CA": (1950, 3450), "PH": (6, 7)}
...
shp_id = next((c for c in gdf.columns if str(c).lower()
              in {"adsfldid", "fieldid", "id"}), None)
shp_nm = next((c for c in gdf.columns if str(c).lower() in {
              "fieldname", "paddock", "name", "label"}), None)
if shp_id:
    gdf["fieldID"] = to_intlike_str(gdf[shp_id])
if shp_nm:
    gdf["fieldName"] = gdf[shp_nm].astype(str).str.strip()
...
if excel_id:
    df["fieldID"] = to_intlike_str(df[excel_id])
if excel_nm:
    df["fieldName"] = df[excel_nm].astype(str).str.strip()
if date_col:
...
for m in METRICS:
    ...
    if c:
        df[m] = make_numeric(df[c])

# latest per field
key = "fieldID" if "fieldID" in gdf.columns and "fieldID" in df.columns else "fieldName"
latest = df.dropna(subset=[key]).sort_values(
    date_col or df.columns[0]).groupby(key, as_index=False).tail(1)

merged = gdf.merge(
    latest[[key]+[m for m in METRICS if m in latest]], on=key, how="left")
...
with open(os.path.join(OUT_DIR, "optima.json"), "w") as f:
    json.dump(OPTIMAL, f)
