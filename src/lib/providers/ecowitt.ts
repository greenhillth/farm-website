...
function mapToWeather(payload: any): Weather {
	...
	const feelsLike =
		pick<number>(data?.outdoor, ['feelsLike', 'feels_like', 'apparent']) ?? outdoorTemp;
	...
	const indoorHumidity =
		pick<number>(data?.indoor, ['humidity', 'rh']) ?? pick<number>(data, ['indoor_humidity']);

	const windDir = pick<number>(data?.wind, ['dir', 'direction', 'degree']);
	const windSpeed =
		pick<number>(data?.wind, ['speed', 'speed_ms', 'avg']) ?? pick<number>(data, ['wind_speed']);
	const windGust =
		pick<number>(data?.wind, ['gust', 'gust_ms', 'max']) ?? pick<number>(data, ['wind_gust']);

	const uvi = pick<number>(data?.solar, ['uvi', 'uv']) ?? pick<number>(data, ['uvi', 'uv']);
	const solar =
		pick<number>(data?.solar, ['solar', 'radiation', 'solar_radiation']) ??
		pick<number>(data, ['solar', 'radiation']);

	const prRel =
		pick<number>(data?.pressure, ['relative', 'rel', 'barometer_rel']) ??
		pick<number>(data, ['pressure_relative', 'barometer_rel']);
	const prAbs =
		pick<number>(data?.pressure, ['absolute', 'abs', 'barometer_abs']) ??
		pick<number>(data, ['pressure_absolute', 'barometer_abs']) ??
		prRel;

	const rainRate =
		pick<number>(data?.rain, ['rate', 'rain_rate']) ?? pick<number>(data, ['rain_rate']);
	const rainDaily =
		pick<number>(data?.rain, ['daily', 'rain_daily']) ?? pick<number>(data, ['rain_daily']);
	const rainEvent =
		pick<number>(data?.rain, ['event', 'rain_event']) ?? pick<number>(data, ['rain_event']);
	const rainHourly =
		pick<number>(data?.rain, ['hourly', 'rain_hourly']) ?? pick<number>(data, ['rain_hourly']);
	const rainWeekly =
		pick<number>(data?.rain, ['weekly', 'rain_weekly']) ?? pick<number>(data, ['rain_weekly']);
	const rainMonthly =
		pick<number>(data?.rain, ['monthly', 'rain_monthly']) ?? pick<number>(data, ['rain_monthly']);
	const rainYearly =
		pick<number>(data?.rain, ['yearly', 'rain_yearly']) ?? pick<number>(data, ['rain_yearly']);
