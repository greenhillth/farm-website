...
export function setActiveMetric(container: HTMLElement, active: string) {
	...
	const labelTarget = container.querySelector<HTMLElement>('[data-active-label]');
...
export function renderLegend(
    ...
    <div class="h-3 border border-border" style="background:${legend.gradientCSS}"></div>
    <div class="flex justify-between text-xs text-muted mt-1">
      <span>${fmt(legend.ticks.vmin)}</span><span>${fmt(legend.ticks.low)}</span>
      <span>${fmt(legend.ticks.mid)}</span><span>${fmt(legend.ticks.high)}</span>
