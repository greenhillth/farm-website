<script lang="ts">
  ...
  const navigationSteps = [
    {
      ...
      detail:
        'The homepage lists each tool as a card. Click a card to open that section in-place.'
    },
    {
  ...
  const features = [
    {
      ...
      summary:
        'Pan and zoom around the property with overlays for soil metrics and paddock notes.',
      href: '/map',
      tips: [
    ...
    },
    {
      title: 'Timesheets',
      summary:
        'Lightweight log of work completed on the farm, ready for more automation later.',
      href: '/timesheet',
      tips: [
  ...
  const supportTips = [
    ...
    },
    {
      title: 'Need a hand?',
      description:
        'If something looks off, note it in the paddock tasks panel or message Tom/Alex directly so they can follow up.'
    }
  ] as const;
</script>
...
<main class="container mx-auto space-y-5 px-4 pb-12">
  ...
  <Panel title="Uploading soil test data">
    <div class="space-y-4">
      ...
      <div class="rounded-lg border border-border/40 bg-white/5 p-4 text-xs leading-relaxed text-muted">
        ...
        <div class="mt-3 overflow-x-auto rounded-md border border-border/30 bg-panel/70 px-3 py-2 font-mono text-[11px] text-muted">
  ...
  <Panel title="Need more help?">
    <div class="grid gap-3 md:grid-cols-3">
      {#each supportTips as tip}
        <article class="rounded-lg border border-border/40 bg-white/5 p-4 text-sm leading-relaxed text-muted">
